...
for folder in ../raw_data_test/*; do
    ...
    tail -n 44 pro-pb.out | grep -E -o 'EGB.{0,21}|ESURF.{0,21}|EELEC.{0,20}|Etot.{0,20}|1-4 EEL.{2,18}' >> pro-pb.txt
    #ligand
    $AMBERHOME/bin/pbsa -i ../../../scripts/pbsa.in -p ../top_crd/lig.top -c ../top_crd/lig.crd -O -o lig-pb.out
    tail -n 44 lig-pb.out | grep -E -o 'EGB.{0,21}|ESURF.{0,21}|EELEC.{0,20}|Etot.{0,20}|1-4 EEL.{2,18}' >> lig-pb.txt
    #complex
    $AMBERHOME/bin/pbsa -i ../../../scripts/pbsa.in -p ../top_crd/complex.top -c ../top_crd/complex.crd -O -o complex-pb.out
    tail -n 44 complex-pb.out | grep -E -o 'EGB.{0,21}|ESURF.{0,21}|EELEC.{0,20}|Etot.{0,20}|1-4 EEL.{2,18}' >> complex-pb.txt
    cd ../..
done
