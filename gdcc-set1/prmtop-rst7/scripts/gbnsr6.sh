...
do
	cd ../$i
	#complex gbnsr6
	$AMBERHOME/bin/gbnsr6 -i ../scripts/gbnsr6.in -O -p complex.prmtop -c complex.crd -o complex-gb.out
	tail -n 15 complex-gb.out | grep -E -o 'EGB.{0,21}|ESURF.{0,21}|EELEC.{0,20}|Etot.{0,20}|1-4 EEL.{2,18}' >> $i-gb.txt	
	
	#Guest gbnsr6
	$AMBERHOME/bin/gbnsr6 -i ../scripts/gbnsr6.in -O -p guest.prmtop -c guest.crd -o guest-gb.out
	tail -n 15 guest-gb.out | grep -E -o 'EGB.{0,21}|ESURF.{0,21}|EELEC.{0,20}|Etot.{0,20}|1-4 EEL.{2,18}' >> $i-guest-gb.txt
	
	#Host gbnsr6
	$AMBERHOME/bin/gbnsr6 -i ../scripts/gbnsr6.in -O -p host.prmtop -c host.crd -o host-gb.out
